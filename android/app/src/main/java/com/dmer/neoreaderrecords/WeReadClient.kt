...
object WeReadClient {
    ...
    fun saveNickname(context: Context, nickname: String) {
        context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
            .edit()
            .putString(KEY_WEREAD_NICKNAME, nickname.trim())
            .apply()
    }
    ...
    fun fetchWallpaperStats(context: Context, apiKey: String, mode: String, baseTimeSeconds: Long? = null): WallpaperStatsResult {
        ...
        return try {
            ...
            val total = normalizeSeconds(json.optLong("totalReadTime", 0L))
            val readDays = json.optInt("readDays", 0)
    ...
    fun fetchBookProgress(context: Context, apiKey: String, bookId: String): BookProgressResult {
        ...
        return try {
            ...
            val totalSeconds = normalizeSeconds(recordReadingTime).takeIf { it > 0L }
                ?: normalizeSeconds(readingTime).takeIf { it > 0L }
                ?: normalizeSeconds(readTime)
    ...
    fun fetchLatestNote(context: Context, apiKey: String, bookId: String): LatestNoteResult {
        ...
        val shortCandidates = candidates.filter { it.text.length <= 30 }
        val selected = if (shortCandidates.isNotEmpty()) {
            shortCandidates.maxByOrNull { it.createTimeMs }
        } else {
